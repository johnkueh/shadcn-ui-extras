...
import { $isListNode, ListNode, insertList, type ListType } from '@lexical/list';
...
export default function ToolbarPlugin() {
  ...
  return (
    <div className="flex flex-nowrap bg-gray-100 rounded-md p-1.5 overflow-y-scroll space-x-1">
      ...
      />
      <ToolbarButton
        onClick={() => (toolbarState.isHeading1 ? formatParagraph() : formatHeading('h1'))}
        isActive={toolbarState.isHeading1}
        icon={Heading1}
      />
      <ToolbarButton
        onClick={() => (toolbarState.isHeading2 ? formatParagraph() : formatHeading('h2'))}
        isActive={toolbarState.isHeading2}
        icon={Heading2}
      />
      <ToolbarButton
        onClick={() => (toolbarState.isQuote ? formatParagraph() : formatQuote())}
      ...
      />
      <ToolbarButton
        onClick={() => (toolbarState.isBulletList ? formatParagraph() : formatList('bullet'))}
        isActive={toolbarState.isBulletList}
        icon={List}
      />
      <ToolbarButton
        onClick={() => (toolbarState.isNumberList ? formatParagraph() : formatList('number'))}
...
function ToolbarButton({ isActive, disabled, onClick, icon: Icon }: ToolbarButton) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={cn(`p-1.5 rounded-md`, isActive && `bg-gray-200`, disabled && `opacity-20`)}>
      <Icon size={16} />
    </button>
  );
}
...
function LinkPopover({ linkUrl, onLinkUrlChange, open, onOpenChange, onSave }: LinkPopover) {
  return (
